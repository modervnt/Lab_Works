...
//Structure of Mistake
/*
class PointOffScreen : public std::runtime_error
{
  private:
    std::string figureName;
  public:
    PointOffScreen(const std::string& figure, const std::string& message)
    : std::runtime_error(message), figureName(figure) {}

    const char* what() const noexcept override {
      return("PointOffScreen:" + figureName + " - " + std::runtime_error::what()).c_str();
    }
};
*/
...
class PointOffScreen : public std::runtime_error {
  ...
  public:
      PointOffScreen(const std::string& figure, const std::string& message)
          : std::runtime_error(message.c_str()), 
            fullMessage("PointOffScreen: " + figure + " - " + message) {}
  ...
  class InvalidFigureParameters : public std::runtime_error {
    private:
        std::string fullMessage;
    
    public:
        InvalidFigureParameters(const std::string& figure, const std::string& message)
            : std::runtime_error(message.c_str()), 
              fullMessage("InvalidFigureParameters: " + figure + " - " + message) {}
...
class cross : public shape { 
  protected:
      point center;  
      int size;      
      std::string figureName; // Nom de la figure pour les exceptions
  
  public:
      // Constructeur avec paramètre
      cross(const std::string& name, point c, int s)
          : center(c), size(s), figureName(name) { 
          if (size <= 0) {
              throw InvalidFigureParameters(figureName, "Size should be greater than 0!");
          }
      }
  
      // Redéfinition des méthodes virtuelles pures avec vérification
      point north() const override {
          point p(center.x, center.y - size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "North point is off-screen!");
          }
          return p;
      }
      ...
      point south() const override {
          point p(center.x, center.y + size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "South point is off-screen!");
          }
          return p;
      }
  
      point east() const override {
          point p(center.x + size / 2, center.y);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "East point is off-screen!");
          }
          return p;
      }
      ...
      point west() const override {
          point p(center.x - size / 2, center.y);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "West point is off-screen!");
          }
          return p;
      }
  
      point neast() const override {
          point p(center.x + size / 2, center.y - size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "Northeast point is off-screen!");
          }
          return p;
      }
      ...
      point seast() const override {
          point p(center.x + size / 2, center.y + size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "Southeast point is off-screen!");
          }
          return p;
      }
      ...
      point nwest() const override {
          point p(center.x - size / 2, center.y - size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "Northwest point is off-screen!");
          }
          return p;
      }
      ...
      point swest() const override {
          point p(center.x - size / 2, center.y + size / 2);
          if (!on_screen(p.x, p.y)) {
              throw PointOffScreen(figureName, "Southwest point is off-screen!");
          }
          return p;
      }
      ...
      void draw() override {
          try {
              ...
              std::cerr << e.what() << '\n';
              // Gérer l'exception ici ou relancer
              throw; // Relance l'exception pour traitement ultérieur
          }
      }
